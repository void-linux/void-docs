...
alternative group. These alternative groups manage the symlinks at
`/usr/lib/jvm/default-jdk` and `/usr/lib/jvm/default-jre`. These are used to set
the `JAVA_HOME` environment variables and add Java utilities to your `PATH` via
a profile script (`/etc/profile.d/jdk.sh`). When installing a Java package for
the first time, you may need to re-login or run `source /etc/profile.d/jdk.sh`
in your terminal session to update these variables.
