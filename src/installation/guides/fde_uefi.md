...
**You must boot the Void Linux live image in UEFI mode or the harddisk installation will not work.**
...
/dev/vda2  264192   4458495   4194304    2G Linux server data
/dev/vda3 4458496 100663262 100399071 45.9G Linux filesystem
```
...
```
# cryptsetup luksOpen /dev/vda3 voidlx
```
...
```
Add the keyfile to the possible keys of /dev/vda3
```
# cryptsetup luksAddKey /dev/vda3 /mnt/boot/volume.key
```
```
# mkfs.vfat /dev/vda1
...
### Encrypted swap /dev/vda2
```
# cryptsetup luksFormat --type luks1 /dev/vda2 --key-file=/mnt/boot/volume.key
# cryptsetup luksOpen /dev/vda2 swap --key-file=/mnt/boot/volume.key
...
* Filesystems
    ...
    Change /dev/vda1 to vfat.  Mount point /boot/efi
  
     "Do you want to create a filesystem on /dev/vda1 ?"
...
When it's done, ***possible with ERROR: faild to install GRUB on /dev/vda!*** (that's OK the void-installer don't know about our crypt setup),
...
```
[xchroot /mnt] # blkid -o value -s UUID /dev/vda3
...
swap            /dev/vda2       /boot/volume.key        luks
```
...
```
[xchroot /mnt] # grub-install /dev/vda
```
